...
ss_true_draws <- ss_van$draws(format = "df")
## Initial population size
gg_van_N0 <- ggplot(data.frame(draw=ss_true_draws$`N0`)) +
...
gg_van_mu_lmr <-ggplot(data.frame(draw=ss_true_draws$`mu_lmr`)) +
  geom_density(aes(x=draw, y=after_stat(density)), color="pink", fill="pink") +
...
gg_van_sigma_lmr <- ggplot(data.frame(draw=ss_true_draws$`sigma_lmr`)) +
  geom_density(aes(x=draw, y=after_stat(density)), color="pink", fill="pink") +
...
gg_van_mu_lfr <- ggplot(data.frame(draw=ss_true_draws$`mu_lfr`)) +
  geom_density(aes(x=draw, y=after_stat(density)), color="pink", fill="pink") +
...
gg_van_sigma_lfr <- ggplot(data.frame(draw=ss_true_draws$`sigma_lfr`)) +
  geom_density(aes(x=draw, y=after_stat(density)), color="pink", fill="pink") +
...
### Abundance
# Figures
pre_abundancy_van <- ss_true_draws |>
  ...
  arrange(index) |>
  select(-index)
post_abundancy_van <- ss_true_draws |>
...
### Stan with seeds (true values)
ss_obs <- list(
  N0_raw = Npre_obs[1] - M_obs[1],
  NB_raw = Npre_obs[2:Ty] - M_obs[2:Ty],
  mu_lmr = mu_lmr_obs,
...
## HMC (Stan) sampling
ss_true <- ss_model$sample(data = ss_data,
                           chains = 4,
                           parallel_chains = 4,
                           refresh = 100,
                           iter_warmup = 1000,
                           iter_sampling = 1000,
                           init = list(ss_obs, ss_obs, ss_obs, ss_obs),
                           save_warmup = TRUE)
# Summaries
ss_true$summary(variables = c("N0",
                              "mu_lmr", "sigma_lmr",
                              "mu_lfr", "sigma_lfr"))
ss_true$summary(variables = "lp__")
### Visualizations
# Data frame with posterior draws when not seeding 
ss_true_draws <- ss_true$draws(format = "df")
## Initial population size
...
### Stan with seeds (MAP)
## HMC (Stan) sampling
ss_map_map <- ss_model$optimize(data = ss_data,
                                jacobian = TRUE,
                                iter = 10000)
ss_map_map$summary(variables = c("N0", 
                                 "mu_lmr", "sigma_lmr", 
                                 "mu_lfr", "sigma_lfr"))
ss_map <- ss_model$sample(data = ss_data,
                          chains = 4,
                          parallel_chains = 4,
                          refresh = 100,
                          iter_warmup = 1000,
                          ...
                          save_warmup = TRUE)
# Summaries
ss_map$summary(variables = c("N0",
                             "mu_lmr", "sigma_lmr",
                             "mu_lfr", "sigma_lfr"))
ss_map$summary(variables = "lp__")
### Visualizations
# Data frame with posterior draws when not seeding 
ss_map_draws <- ss_map$draws(format = "df")
## Initial population size
gg_map_N0 <- ggplot(data.frame(draw=ss_map_draws$`N0`)) +
  geom_density(aes(x=draw, y=after_stat(density)), color="pink", fill="pink") +
  stat_function(fun = dlnorm, args = list(meanlog=pm_LN0, sdlog=psd_LN0), colour="black", linewidth=1) +
  xlim(0, exp(pm_LN0+0.5*(psd_LN0**2)) + 3*sqrt((exp(psd_LN0**2)-1)*exp(2*pm_LN0+(psd_LN0**2)))) +
  geom_vline(xintercept = N0_obs, color="blue") +
  ...
  labs(title = TeX("Prior and posterior distribution of $N^{pre}_{1}$"),
       y = TeX("Density"), x = TeX("$N^{pre}_{1}$")) +
  theme(plot.title = element_text(size = 18, face = "bold", hjust = 0.5))
grid.arrange(gg_map_N0)
## Hyperparameters
# Mean of log mortality rates
gg_map_mu_lmr <-ggplot(data.frame(draw=ss_map_draws$`mu_lmr`)) +
  geom_density(aes(x=draw, y=after_stat(density)), color="pink", fill="pink") +
  stat_function(fun = dnorm, args = list(mean=pm_mu_lmr, sd=psd_mu_lmr), colour="black", linewidth=1) +
  xlim(min(pm_mu_lmr - 6*psd_mu_lmr, min(ss_map_draws$`mu_lmr`)), max(pm_mu_lmr + 6*psd_mu_lmr, max(ss_map_draws$`mu_lmr`))) +
  geom_vline(xintercept = mu_lmr_obs, color="blue") +
  theme_minimal() +
  labs(title = TeX("Prior and posterior distribution of $\\mu_{lmr}$"),
       y = TeX("Density"), x = TeX("$\\mu_{lmr}$")) +
  theme(plot.title = element_text(size = 18, face = "bold", hjust = 0.5))
# Standard deviation of log mortality rates
gg_map_sigma_lmr <- ggplot(data.frame(draw=ss_map_draws$`sigma_lmr`)) +
  geom_density(aes(x=draw, y=after_stat(density)), color="pink", fill="pink") +
  stat_function(fun = dtnorm, args = list(mean=pm_sigma_lmr, sd=psd_sigma_lmr), colour="black", linewidth=1) +
  xlim(max(0, pm_sigma_lmr - 6*psd_sigma_lmr), max(pm_sigma_lmr + 6*psd_sigma_lmr, max(ss_map_draws$`sigma_lmr`))) +
  geom_vline(xintercept = sigma_lmr_obs, color="blue") +
  theme_minimal() +
  labs(title = TeX("Prior and posterior distribution of $\\sigma_{lbr}$"),
       y = TeX("Density"), x = TeX("$\\sigma_{lbr}$")) +
  theme(plot.title = element_text(size = 18, face = "bold", hjust = 0.5))
# Mean of log fertility rates
gg_map_mu_lfr <- ggplot(data.frame(draw=ss_map_draws$`mu_lfr`)) +
  geom_density(aes(x=draw, y=after_stat(density)), color="pink", fill="pink") +
  stat_function(fun = dnorm, args = list(mean=pm_mu_lfr, sd=psd_mu_lfr), colour="black", linewidth=1) +
  xlim(min(pm_mu_lfr - 6*psd_mu_lfr, min(ss_map_draws$`mu_lfr`)), max(pm_mu_lfr + 6*psd_mu_lfr, max(ss_map_draws$`mu_lfr`))) +
  geom_vline(xintercept = mu_lfr_obs, color="blue") +
  theme_minimal() +
  labs(title = TeX("Prior and posterior distribution of $\\mu_{lfr}$"),
       y = TeX("Density"), x = TeX("$\\mu_{lfr}$")) +
  theme(plot.title = element_text(size = 18, face = "bold", hjust = 0.5))
# Standard deviation of log fertility rates
gg_map_sigma_lfr <- ggplot(data.frame(draw=ss_map_draws$`sigma_lfr`)) +
  geom_density(aes(x=draw, y=after_stat(density)), color="pink", fill="pink") +
  stat_function(fun = dtnorm, args = list(mean=pm_sigma_lfr, sd=psd_sigma_lfr), colour="black", linewidth=1) +
  xlim(max(0, pm_sigma_lfr - 6*psd_sigma_lfr), max(pm_sigma_lfr + 6*psd_sigma_lfr, max(ss_map_draws$`sigma_lfr`))) +
  geom_vline(xintercept = sigma_lfr_obs, color="blue") +
  theme_minimal() +
  labs(title = TeX("Prior and posterior distribution of $\\sigma_{lfr}$"),
       y = TeX("Density"), x = TeX("$\\sigma_{lfr}$")) +
  theme(plot.title = element_text(size = 18, face = "bold", hjust = 0.5))
# Plots
grid.arrange(gg_map_mu_lfr, gg_map_sigma_lfr,
             gg_map_mu_lmr, gg_map_sigma_lmr,
             nrow = 2, ncol = 2,
             layout_matrix = rbind(c(1,2), 
                                   c(3,4)))
### Abundance
# Figures
pre_abundancy_map <- ss_map_draws |>
  select(starts_with("Npre")) |>
  pivot_longer(everything(), names_to = "parameter", values_to = "value") |>
  mutate(index = as.numeric(str_extract(parameter, "(?<=\\[)\\d+(?=\\])"))) |>
  group_by(parameter, index) |>
  ...
  arrange(index) |>
  select(-index)
post_abundancy_map <- ss_map_draws |>
  select(starts_with("Npost")) |>
  pivot_longer(everything(), names_to = "parameter", values_to = "value") |>
  mutate(index = as.numeric(str_extract(parameter, "(?<=\\[)\\d+(?=\\])"))) |>
  group_by(parameter, index) |>
  ...
  select(-index)
# Plots
gg_map_Npre <- ggplot(pre_abundancy_map, aes(x = 1:Ty, y = mean)) +
  geom_ribbon(aes(ymin = q2.5, ymax = q97.5), 
              fill = "pink", alpha = 0.5) +
  geom_line(color = "magenta", linewidth = 1) +
  geom_point(aes(x = 1:Ty, y = Npre_obs)) +
  ylim(0, max(pre_abundancy_map$q97.5)) +
  theme_minimal() +
  labs(title = TeX("Posterior of the pre-mortality population"),
       y = TeX("Total population"), x = "Year") +
  theme(
...
gg_map_Npost <- ggplot(post_abundancy_map, aes(x = 1:Ty, y = mean)) +
  geom_ribbon(aes(ymin = q2.5, ymax = q97.5), 
              fill = "pink", alpha = 0.5) +
  geom_line(color = "magenta", linewidth = 1) +
  geom_point(aes(x = 1:Ty, y = Npost_obs)) +
...
grid.arrange(gg_map_Npre, gg_map_Npost, nrow = 1, ncol = 2)
### Fertility rates
# Figures
fr_map <- ss_map_draws |>
  select(starts_with("fr")) |>
  ...
  select(-index)
# Plots
gg_map_fr <- ggplot(fr_map, aes(x = 1:Ty, y = mean)) +
  geom_ribbon(aes(ymin = q2.5, ymax = q97.5), 
              fill = "pink", alpha = 0.5) +
  geom_line(color = "magenta", linewidth = 1) +
  geom_point(aes(x = 1:Ty, y = fr_obs)) +
...
grid.arrange(gg_map_fr, nrow = 1, ncol = 1)
## Mortality rates
mr_map <- ss_map_draws |>
  select(starts_with("mr")) |>
  pivot_longer(everything(), names_to = "parameter", values_to = "value") |>
  ...
  select(-index)
# Plots
gg_map_mr <- ggplot(mr_van, aes(x = 1:Ty, y = mean)) +
  geom_ribbon(aes(ymin = q2.5, ymax = q97.5), 
              fill = "pink", alpha = 0.5) +
  geom_line(color = "magenta", linewidth = 1) +
  geom_point(aes(x = 1:Ty, y = mr_obs)) +
...
grid.arrange(gg_map_mr, nrow = 1, ncol = 1)
